...
def get_profile_benefit_ordering(clprofile,bugprofile,bugdf,cleandf):
	...
	for profile in clprofile.keys():
		if profile=='conformance':
			benefit[profile]=clprofile["conformance"].evaluate(bugdf).avg_violation
			continue
		print(profile,clprofile[profile],bugprofile[profile])
		if 'max' in profile[0]:
			benefit[profile]=get_profile_distance_normalized(clprofile[profile],bugprofile[profile])
		else:	
			benefit[profile]=get_profile_distance_corr(clprofile[profile],bugprofile[profile])
...
noisydf=pd.read_csv('../datasets/flights/train.csv',delimiter=',')
print (noisydf.shape)
...
fout=open("dp.txt","w")
fout.write((num_interventions)+" "+str(end-start))
fout.close()

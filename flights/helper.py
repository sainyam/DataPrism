...
class Dataset:
	...
	def populate_profiles(self):
		...
		for column in self.df.columns:
			...
			#########Heuristic to decide if it is categorical and will vary for each dataset
			##############
			if len(uniq_values) < 50 and len(values)>2*len(uniq_values):#Check if a column is categorical
		...
		while i<len(numerical_columns):
			...
			while j<len(numerical_columns):
				...
				profile_map[("corr",numerical_columns[i],numerical_columns[j])]=P.correlation(self.df[numerical_columns[i]].fillna(0),self.df[numerical_columns[j]].fillna(0))
				j+=1
			i+=1
		...
		while i<len(collst):
		    ...
		    while j<len(collst):
		        ...
		        #print ("functional",collst[i],collst[j], profile_map[("functional",collst[i],collst[j])])

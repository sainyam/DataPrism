...
class Profile:
	...
	def apply_transformation (self,lst,col,prof,rangeval,buggyprofilelst,cleanprofilelst):
		if 'corr' in prof:
			return self.shuffle_transform(lst)
		if 'max' in prof:
			maxval= (rangeval)
			minval = (buggyprofilelst[('min',col)],cleanprofilelst[('min',col)])

			add = minval[1]-minval[0]

			mult= maxval[1]*1.0/(maxval[0]+add)
			
			return self.linear_transform(lst,add,mult)
		if 'min' in prof:
			minval= (rangeval)
			maxval = (buggyprofilelst[('max',col)],cleanprofilelst[('max',col)])
...
#We get a list of benefit which is list pairs of profile and columns
#we need to choose the most beneficial among these
#Generally column is the one which is present maximum number of times
#Profile is most beneficial for that column
#we can have a window and choose within that!!!!
